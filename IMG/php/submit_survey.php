...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("
        INSERT INTO surveys 
            (user_id, product_name, commodity_type, variety, price, quantity_type, quality, note, address, product_image, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
    ");
    ...
    if ($stmt->execute()) {
        $stmt->close();
        $conn->close();
        // ✅ Redirect to the correct product list page
